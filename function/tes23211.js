    ...
    // Step 4 specific - 11 second wait before allowing click
    function initStep4Wait() {
        if (readFlag('step4_done') || readFlag('step4_wait_complete')) return;
        
        // Reset step 4 button state
        btn(4).disabled = true;
        btn(4).textContent = 'Step 4 — Please wait 11s';
        
        // Start countdown
        let waitTime = 11;
        noteEl(4).textContent = `Please wait ${waitTime} seconds before proceeding...`;
        
        const countdown = setInterval(() => {
            waitTime -= 1;
            
            if (waitTime > 0) {
                btn(4).textContent = `Step 4 — Please wait ${waitTime}s`;
                noteEl(4).textContent = `Please wait ${waitTime} seconds before proceeding...`;
            } else {
                clearInterval(countdown);
                setFlag('step4_wait_complete', true);
                btn(4).disabled = false;
                btn(4).textContent = 'Step 4 — Click to Continue';
                noteEl(4).textContent = 'Click the button to proceed to the shortlink.';
            }
        }, 1000);
    }
    ...
    (function checkReturnOnLoad() {
        try {
            ...
            updateUI();
            // Initialize step 4 wait if needed
            if (readFlag('step3_done') && !readFlag('step4_done')) {
                initStep4Wait();
            }
        }
    })();
        ...
        for (let i = 1; i <= 5; i++) {
            ...
            if (done) {
                ...
                if (prevDone) {
                    ...
                    if (i === 4 && !readFlag('step4_wait_complete') && !readFlag('step4_done')) {
                        initStep4Wait();
                    }
                } else {
                    stepEl.classList.remove('active');
            ...
            // Enable button only if previous step is done, current step is not done, and for step 4, wait is complete
            let allow = (i === 1 || prevDone) && !done;
            if (i === 4) {
                allow = allow && readFlag('step4_wait_complete');
            }
